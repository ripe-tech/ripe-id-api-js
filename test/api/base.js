...
describe("AccountAPI", function() {
    ...
    describe("#oauthAuthorize()", function() {
        it("should be able to generate a simple authorize URL", async () => {
            let result;
            const api = new ripe.API(undefined, undefined, "dummy", "dummy", "http://dummy.com/redirect");
            result = await api.oauthAuthorize("data");
            assert.strictEqual(result, "https://id.platforme.com/oauth2/auth?client_id=dummy&redirect_uri=http%3A%2F%2Fdummy.com%2Fredirect&response_type=code&scope=account.me%20account.acl&state=data");
        });
    });
});
